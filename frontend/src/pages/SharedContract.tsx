  ...
  return (
    <div style={{ border: `1px solid ${open ? cfg.color + '30' : 'var(--border)'}`, borderRadius: 'var(--radius-sm)', overflow: 'hidden', marginBottom: 8, transition: 'border-color 0.2s' }}>
      <button onClick={() => setOpen(!open)} style={{ width: '100%', background: open ? `${cfg.color}06` : 'var(--bg-card)', border: 'none', cursor: 'pointer', padding: '13px 16px', display: 'flex', alignItems: 'center', gap: 12, transition: 'background 0.2s' }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.color}20`, whiteSpace: 'nowrap' }}>{cfg.label}</span>
        ...
        <div style={{ padding: '14px 16px', borderTop: `1px solid ${cfg.color}18`, background: 'var(--bg-elevated)' }}>
          {clause.clause_text && (
            <div style={{ marginBottom: 12, padding: '9px 12px', background: 'var(--bg-card)', borderRadius: 7, borderLeft: `3px solid ${cfg.color}` }}>
              <p style={{ fontSize: 14, color: 'var(--text-muted)', fontStyle: 'italic', lineHeight: 1.7 }}>"{clause.clause_text}"</p>
            </div>
          )}
          {clause.explanation && <p style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.7, marginBottom: 10 }}>{clause.explanation}</p>}
          {clause.regulatory_reference && (
            <div style={{ padding: '8px 12px', background: 'rgba(59,130,246,0.05)', borderRadius: 7, border: '1px solid var(--border-blue)', marginBottom: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--primary)', marginBottom: 3, display: 'flex', alignItems: 'center', gap: 4 }}><Scale size={9} /> LEGAL REFERENCE</div>
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.5 }}>{clause.regulatory_reference}</p>
            </div>
          )}
          {clause.recommendation && (
            <div style={{ padding: '9px 12px', background: `${cfg.color}08`, borderRadius: 7, border: `1px solid ${cfg.color}20` }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: cfg.color, marginBottom: 3 }}>💡 RECOMMENDATION</div>
              <p style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.6, fontWeight: 500 }}>{clause.recommendation}</p>
            </div>
          )}
        </div>
      )}
    ...
    <div style={{ background: 'var(--bg-deep)', minHeight: '100vh', padding: '0 0 60px' }}>
      ...
      <div style={{ background: 'rgba(8,13,26,0.95)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', position: 'sticky', top: 0, zIndex: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontWeight: 900, fontSize: 16 }}>AQD <span style={{ color: 'var(--gold)' }}>·</span> <span style={{ fontFamily: 'Playfair Display, serif' }}>عقد</span></span>
          <span style={{ fontSize: 13, color: 'var(--text-dim)', padding: '2px 8px', borderRadius: 99, background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>Shared Report</span>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => window.open(`${backendBase}/reports/${contract.id}/download/`, '_blank')} className="btn btn-ghost" style={{ fontSize: 14, padding: '7px 14px' }}>
            <Download size={12} /> Download PDF
          </button>
          <Link to="/register" className="btn btn-primary" style={{ textDecoration: 'none', fontSize: 14, padding: '7px 14px' }}>
      ...
      <div style={{ maxWidth: 860, margin: '32px auto', padding: '0 24px' }}>
        ...
        <div className="card" style={{ marginBottom: 20, border: `1px solid ${riskColor}25` }}>
          <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <span style={{ fontSize: 13, padding: '2px 8px', borderRadius: 99, background: 'var(--bg-elevated)', color: 'var(--text-muted)' }}>{contract.language === 'ar' ? '🇸🇦 Arabic' : '🇬🇧 English'}</span>
              </div>
            </div>
            ...
            <svg width={90} height={90} viewBox="0 0 90 90">
              ...
              <text x={45} y={49} textAnchor="middle" fill={riskColor} fontSize={18} fontWeight={900} fontFamily="Cairo">{contract.compliance_score}%</text>
            </svg>

            <div style={{ padding: '12px 18px', borderRadius: 10, background: `${riskColor}10`, border: `2px solid ${riskColor}40`, textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: riskColor, fontWeight: 800, letterSpacing: '1px' }}>{riskLabel}</div>
          ...
          {contract.executive_summary && (
            <div style={{ marginTop: 18, paddingTop: 16, borderTop: '1px solid var(--border)', fontSize: 15, color: 'var(--text)', lineHeight: 1.8 }}>
              {contract.executive_summary}
            </div>
          )}
        </div>
        ...
        {[contract.employer_name, contract.job_title, contract.basic_salary, contract.work_location].some(Boolean) && (
          <div className="card" style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-dim)', letterSpacing: '1px', marginBottom: 14 }}>CONTRACT DETAILS</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 10 }}>
              {[
                ...
                <div key={label} style={{ padding: '10px 12px', background: 'var(--bg-elevated)', borderRadius: 8, border: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 700, letterSpacing: '0.5px', marginBottom: 3 }}>{label.toUpperCase()}</div>
        ...
        {/* Disclaimer */}
        <div style={{ padding: '14px 16px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.7 }}>
        ...
        <div style={{ marginTop: 24, textAlign: 'center', padding: '28px', background: 'linear-gradient(135deg, rgba(59,130,246,0.06), rgba(234,179,8,0.06))', border: '1px solid var(--border)', borderRadius: 14 }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 15, marginBottom: 18 }}>Get AI-powered Saudi Labor Law compliance review in seconds — free to start.</p>
          <Link to="/register" className="btn btn-gold" style={{ textDecoration: 'none', display: 'inline-flex', fontSize: 15, padding: '11px 24px' }}>
            Get Started Free →
          </Link>
        </div>
      </div>
