  ...
  return (
    <div style={{ padding: '32px', maxWidth: 900, margin: '0 auto' }} className="animate-fade-up">
      <div style={{ marginBottom: 32, textAlign: isRTL ? 'right' : 'left' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>{labels.subtitle}</p>
      </div>
      ...
      {contracts.length < 2 ? (
        <div className="card" style={{ textAlign: 'center', padding: '60px 32px' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 20 }}>{labels.need2sub}</p>
          <a href="/upload" className="btn btn-primary" style={{ display: 'inline-flex', fontSize: 13, textDecoration: 'none' }}>{labels.analyzeFirst}</a>
        </div>
      ) : (
        <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
              {[c1, c2].map((id, i) => {
                ...
                if (!c) return <div key={i} className="card" style={{ opacity: 0.4, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 100 }}>
                  <span style={{ color: 'var(--text-dim)', fontSize: 13 }}>{labels.select}</span>
                </div>;
                  ...
                  <div key={i} className="card" style={{ border: `1px solid ${risk}25` }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 12, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0, textAlign: isRTL ? 'right' : 'left' }}>
                        <div style={{ fontSize: 13, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.file_name}</div>
                        <div style={{ fontSize: 10, color: 'var(--text-dim)', marginTop: 2 }}>{c.employer_name || c.contract_type_display}</div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>{c.compliant_count}✓ · {c.attention_count}⚠ · {c.non_compliant_count}✗</div>
                    </div>
                  </div>
                );
              })}
          ...
          {error && <div style={{ padding: '10px 14px', background: 'var(--red-bg)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, fontSize: 12, color: 'var(--red)', marginBottom: 16 }}>{error}</div>}

          <button onClick={compare} disabled={!c1 || !c2 || c1 === c2 || comparing} className="btn btn-primary"
            style={{ width: '100%', padding: '14px', fontSize: 14, opacity: (!c1 || !c2 || c1 === c2) ? 0.4 : 1 }}>
          ...
          {result && (
            <div className="card" style={{ marginTop: 24, border: '1px solid var(--border-blue)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700 }}>{labels.result}</span>
              </div>
              <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.9, whiteSpace: 'pre-wrap', textAlign: isRTL ? 'right' : 'left' }}>{result}</div>
            </div>
          )}
        </>
      )}
...
function ContractSelect({ label, value, onChange, contracts, exclude, lang, isRTL, placeholder }: any) {
  ...
  return (
    <div>
      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-dim)', letterSpacing: isRTL ? 0 : '1px', marginBottom: 8, textAlign: isRTL ? 'right' : 'left' }}>{label}</div>
      <div style={{ position: 'relative' }}>
        <button onClick={() => setOpen(!open)} style={{ width: '100%', background: 'var(--bg-elevated)', border: '1px solid var(--border-light)', borderRadius: 'var(--radius-sm)', padding: '12px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: 'var(--text)', fontFamily: 'Cairo, Outfit, sans-serif', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
          <span style={{ fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '85%', color: selected ? 'var(--text)' : 'var(--text-dim)' }}>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.file_name}</div>
                    <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>{c.compliance_score}% · {c.employer_name || c.contract_type_display}</div>
                  </div>
                </button>
              );
            })}
